...
import { User, ShieldCheck, Truck, LogOut, PhoneCall } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    driverService.getMe()
      .then((d) => { if (!cancelled) setDriver(d); })
      .catch(() => {})
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [pathname]);

  const userName = driver?.name || user?.name || 'Driver Name';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="p-6 space-y-6 pb-32">
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col items-center text-center transition-all duration-300">
          <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center mb-4 border-4 border-white shadow-lg relative">
            {loading ? (
              <div className="w-10 h-10 rounded-full bg-[#f7f7f7] animate-pulse" />
            ) : (
            ...
            <>
              ...
              <div className="flex gap-2 mt-4">
                <span className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg text-[10px] font-black tracking-widest uppercase border border-slate-200 flex items-center gap-1.5">
                  <ShieldCheck className="w-3.5 h-3.5" /> {driver?.licenseNumber || 'CDL-A'}
                </span>
                <span className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg text-[10px] font-black tracking-widest uppercase border border-slate-200 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-4 transition-all duration-300">
          <div className="bg-emerald-50 p-5 rounded-3xl border border-emerald-100 flex flex-col">
            {loading ? (
              <div className="h-9 w-16 rounded bg-emerald-100 animate-pulse" />
            ) : (
              <span className="text-3xl font-black text-emerald-700">{driver ? `${(driver.onTimeRate * 100).toFixed(0)}%` : '0%'}</span>
            )}
          ...
          <div className="bg-blue-50 p-5 rounded-3xl border border-blue-100 flex flex-col">
            {loading ? (
              <div className="h-9 w-12 rounded bg-blue-100 animate-pulse" />
            ) : (
              <span className="text-3xl font-black text-blue-700">{driver?.rating?.toFixed(1) || '0.0'}</span>
            )}
            <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mt-1">Driver Rating</span>
          </div>
        </div>

        <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm transition-all duration-300">
        ...
        <div className="flex flex-col gap-2 pt-4 border-t border-slate-200 transition-all duration-300">
          ...
          <Button onClick={handleLogout} variant="ghost" className="w-full justify-start h-12 text-red-600 hover:bg-red-50 hover:text-red-700 font-bold mt-2">
