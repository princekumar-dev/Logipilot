...
function SkeletonChatBubble({ align = 'left' }: { align?: 'left' | 'right' }) {
  return (
    <div className={`flex gap-4 max-w-3xl ${align === 'right' ? 'ml-auto justify-end' : ''} transition-all duration-300`}>
      {align === 'left' && (
        <div className="w-10 h-10 rounded-xl bg-[#f7f7f7] animate-pulse flex-shrink-0" />
      )}
      <div className={`${align === 'right' ? 'bg-slate-200' : 'bg-white border border-slate-200'} p-5 rounded-2xl ${align === 'right' ? 'rounded-tr-sm' : 'rounded-tl-sm'} space-y-3 w-full max-w-md`}>
        <div className={`h-3 rounded ${align === 'right' ? 'bg-slate-300' : 'bg-[#f7f7f7]'} animate-pulse w-24`} />
        <div className={`h-3 rounded ${align === 'right' ? 'bg-slate-300' : 'bg-[#f7f7f7]'} animate-pulse w-full`} />
        <div className={`h-3 rounded ${align === 'right' ? 'bg-slate-300' : 'bg-[#f7f7f7]'} animate-pulse w-3/4`} />
      </div>
      {align === 'right' && (
...
export default function CopilotPage() {
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col h-[calc(100vh-4rem)] max-w-[1600px] mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 transition-all duration-300">
        <div>
      ...
      <div className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex mt-2 transition-all duration-300">
        <div className="w-72 border-r border-slate-100 bg-slate-50/50 p-4 hidden md:flex flex-col gap-2">
          ...
          {loading ? (
            <div className="space-y-2">
              {[0, 1, 2].map(i => (
                <div key={i} className="p-3 rounded-xl bg-white border border-slate-200 flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#f7f7f7] animate-pulse" />
        ...
        <div className="flex-1 flex flex-col bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed relative">
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 z-10 relative">
            {loading ? (
              ...
              <>
                <div className="flex gap-4 max-w-3xl transition-all duration-300">
                  ...
                  <div className="bg-white border border-slate-200 shadow-sm p-5 rounded-2xl rounded-tl-sm text-sm text-slate-700">
                    ...
                    <p className="font-medium text-base leading-relaxed">I&apos;ve analyzed the Chennai-Bangalore route for today. Google Maps traffic data indicates a major blockade near Krishnagiri due to roadwork.</p>
                ...
                <div className="flex gap-4 max-w-3xl ml-auto justify-end transition-all duration-300">
